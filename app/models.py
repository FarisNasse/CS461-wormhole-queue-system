...
class Ticket(db.Model):
    ...
    status: orm.Mapped[str] = orm.mapped_column(
        sa.String(20), index=True, default="live"
    )
    created_at: orm.Mapped[datetime] = orm.mapped_column(
        index=True, default=lambda: datetime.now(UTC)
    )
    time_resolved: orm.Mapped[datetime | None] = orm.mapped_column(default=None)
    number_of_students: orm.Mapped[int | None] = orm.mapped_column(default=1)
    wa_id: orm.Mapped[int | None] = orm.mapped_column(
        sa.ForeignKey("users.id"), default=None, index=True
    )
    wormhole_assistant: orm.Mapped[User | None] = orm.relationship(
    ...
    def mark_resolved(self, user: Optional["User"] = None):
        ...
        self.time_resolved = datetime.now(UTC)
