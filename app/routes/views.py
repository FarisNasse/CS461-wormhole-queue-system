...
from flask import Blueprint, render_template, request, url_for, abort, session, redirect, flash
...
def queue():
    ...
    open_tickets = Ticket.query.filter_by(status='live', wa_id=None).order_by(Ticket.created_at).all()
    current_tickets = Ticket.query.filter_by(status='current').order_by(Ticket.created_at).all()
    ...
    return render_template('queue.html', ol=ol, cul=cul, cll=cll, user=SimpleNamespace(username='admin'))
